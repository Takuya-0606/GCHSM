...
def gen_surface(mol, ng=302, rad=modified_Bondi, vdw_scale=1.2, cavity_coords=None):
    '''J. Phys. Chem. A 1999, 103, 11060-11079'''
    unit_sphere = gen_grid.MakeAngularGrid(ng)
    if cavity_coords is None:
        atom_coords = mol.atom_coords(unit='B')
    else:
        atom_coords = numpy.asarray(cavity_coords, dtype=float)
        if atom_coords.shape != (mol.natm, 3):
            raise ValueError('cavity_coords must have shape (natm, 3); '
                             f'got {atom_coords.shape} for natm={mol.natm}')
    #atom_coords = mol.atom_coords(unit='B')
    ...
    print(f"Surface area [Bohr**2] = {numpy.sum(area):.9f}")
    ...
    surface = {
        ...
        'R_in_J': R_in_J,
        'R_sw_J': R_sw_J,
        'atom_coords': numpy.array(atom_coords, copy=True)
    }
    return surface
...
class PCM(lib.StreamObject):
    ...
    def __init__(self, mol):
        ...
        self.v = None
        self._dm = None
        self._cavity_coords = numpy.array(mol.atom_coords(unit='B'), copy=True)

    @property
    def cavity_coords(self):
        '''Return a copy of the coordinates used to generate the PCM cavity.'''
        return numpy.array(self._cavity_coords, copy=True)

    @cavity_coords.setter
    def cavity_coords(self, coords):
        coords = numpy.asarray(coords, dtype=float)
        if coords.shape != (self.mol.natm, 3):
            raise ValueError('cavity_coords must have shape (natm, 3); '
                             f'got {coords.shape} for natm={self.mol.natm}')
        self._cavity_coords = numpy.array(coords, copy=True)
        self.reset()

    def set_cavity_coords(self, coords):
        '''Backward compatible helper to update :attr:`cavity_coords`.'''
        self.cavity_coords = coords

    def get_cavity_coords(self):
        '''Return the coordinates defining the PCM cavity.'''
        return self._cavity_coords
    ...
    def reset(self, mol=None):
        if mol is not None:
            self.mol = mol
            if self._cavity_coords.shape != (mol.natm, 3):
                self._cavity_coords = numpy.array(mol.atom_coords(unit='B'), copy=True)
    ...
    def build(self, ng=None):
        ...
        self.surface = gen_surface(mol, rad=radii_table, ng=ng,
                                   cavity_coords=self.get_cavity_coords())
        self._intermediates = {}
